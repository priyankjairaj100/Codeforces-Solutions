...
int main(){
    ...
    int f = 0;
    rep(i,0,cl,1) rep(j,0,acr,1) { int xd = acr + both - i - j;if(xd%2 != 0)continue; int xd1 = xd/2; if(xd1 >= 0 and xd1 <= both and (i + j + xd1 <= n1) and (cl-i+acr-j+both-xd1 <= n1)){sb=i;sc=j;sd=xd1; sa = n1 - (sb+sc+sd);f = 1; break;} }
