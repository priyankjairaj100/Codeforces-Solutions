...
int main() {
	...
	vector<LL> tp; rep(i,1,32,1) tp.pb(1LL<<i);
	map <LL,LL> m; rep(i,1,n,1) m[a[i]]++; LL ans =0;
	rep(i,1,n,1){
		for(auto j:tp){
			if(j-a[i] == a[i] and m[j-a[i]] > 1) {ans++; break;}
			else if(j-a[i]!=a[i] and m.find(j-a[i])!=m.end()){ ans++;break;}
		}
	}
	cout<<n-ans<<endl;
	return 0;
