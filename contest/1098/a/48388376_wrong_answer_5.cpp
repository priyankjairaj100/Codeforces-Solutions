...
void dfs(int x, int pp){
    for(auto y: g[x]){
        if(y!=pp){
            if(s[y] == -1) {a[y] = 0; s[y] = s[x];}
            else if(s[y]<s[x]) flag = 1;
            else if(s[y]>=s[x]) a[y] = s[y]-s[x];
            dfs(y,x);
        }
    }
}
...
int main(){
	fastio
    int n; cin>>n;
    rep(i,1,n-1,1){ int x; cin>>x; g[x].pb(i+1); g[i+1].pb(x); }
    rep(i,1,n,1) cin>>s[i]; a[1] = (s[1]==-1)? 0: s[1];
    dfs(1,0);
    if(flag) cout<<"-1"<<endl;
