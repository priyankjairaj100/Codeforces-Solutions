...
LL v[300005];
LL pw[300005], pw3[300005];
vector<int> g[300005];

void dfs1(int x, int sign){
	v[x] = -sign; if(v[x]==1) b++; else w++;
	for(auto i:g[x]) {if(!v[i]) dfs1(i,v[x]); else if(v[i] == v[x]) flag = 1;} 
}

void dfs(){
	LL ans = 1;
	rep(i,1,n,1) v[i] = 0; isol = 0;
	rep(i,1,n,1) if(!v[i]){ 
		if(flag) break;
		b=0, w=0;
		dfs1(i, 1);
...
int main(){
	fastio
	pw[0] = 1; rep(i,1,300003,1) pw[i]= (pw[i-1]*2)%md;
	pw3[0] = 1; rep(i,1,300003,1) pw3[i]= (pw3[i-1]*3)%md;
	int t; cin>>t;
	while(t--){
