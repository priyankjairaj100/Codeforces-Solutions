...
void dfs(vector<int> g[300005])
{
	...
	if(isol) {rep(i,1,isol,1) { ta*=3; ta%=md; } ans+=ta,ans%=md,ta=1,b=0,w=0;}
	rep(i,1,n,1) if(!v[i] and g[i].size()){
		dfs1(i, 1, g);
		if(b){rep(i,1,b,1) {ta*=2; ta%=md;} ans+=ta,ans%=md,ta=1;}
		if(w){rep(i,1,w,1) {ta*=2; ta%=md;} ans+=ta,ans%=md,ta=1;}
	}
