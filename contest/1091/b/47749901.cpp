...
int main(){
	fastio
	int n; cin>>n; int a[n+1],b[n+1], x[n+1], y[n+1];
    ...
    rep(i,1,n,1){
        rep(j,1,n,1){
            if(i==1) s.insert({x[i]+a[j],y[i]+b[j]});
            else t.insert({x[i]+a[j],y[i]+b[j]});
        }
        if(i>1){
            set<pair<int, int>> st;
            for(auto c:t) if(s.find(c)!=s.end()) st.insert(c);
            for(auto c:s) if(t.find(c)!=t.end()) st.insert(c);
            s.clear(); t.clear();
