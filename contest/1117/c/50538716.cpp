...
LL x1, y1, x2, y2, n, x, y, u[100005], r[100005], ans = 1e18;

LL ok(LL d){
    LL xf = x2 - x1, yf = y2 - y1;
    LL wx = (r[n] * ((d-1)/n)) + r[(d-1)%n + 1], wy = (u[n] * ((d-1)/n)) + u[(d-1)%n + 1];
    LL d1 = abs(xf - wx) + abs(yf - wy);
    //cout<<d<<" "<<wx<<" "<<wy<<endl;
...
int main(){
    ...
    rep(i,1,n,1){ u[i]+=u[i-1]; r[i]+=r[i-1];}
    bs(1, 1e18);
    if(ans == 1e18){ cout<<-1<<endl; return 0;}
    cout<<ans<<endl;
    return 0; 
}