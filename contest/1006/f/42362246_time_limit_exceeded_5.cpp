...
void fb(LL x, LL y, LL val, LL cnt){
    ...
    if(x<n) fb(x+1,y,val,cnt+1);
    if(y<m) fb(x,y+1,val,cnt+1);
}
void fa(LL x, LL y, LL val, LL cnt){
    if(cnt == n+m-2-half){
        if(v[x][y].count(k ^ val)) {ans+=v[x][y][k^val]; return;}
    }
    if(x > 1) fa(x-1,y,val^a[x][y], cnt+1);
    if(y > 1) fa(x,y-1,val^a[x][y],cnt+1);
}
int main() {
    fastio
    cin>>n>>m>>k;
    fr1(i,n) for(LL j =1; j<=m;j++) cin>>a[i][j];
    half = (n+m-2)/2;
    fb(1,1,0,0); fa(n,m,0,0);
    cout<<ans<<endl;
	return 0;
}