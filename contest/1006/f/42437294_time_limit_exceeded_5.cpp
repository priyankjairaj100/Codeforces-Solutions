...
LL n,m,k,ans,half;
LL a[22][22];
void fa(LL x, LL y, LL val, LL cnt){
    val^= a[x][y];
    if(cnt == half){
        ++v[x][y][val];
        return;
    }
    if(x<n-1) fa(x+1,y,val,cnt+1);
    if(y<m-1) fa(x,y+1,val,cnt+1);
}
void fb(LL x, LL y, LL val, LL cnt){
    if(cnt == n+m-2-half){
        if(v[x][y].count(k ^ val)) {ans+=v[x][y][k^val]; return;}
    }
    if(x >0) fb(x-1,y,val^a[x][y], cnt+1);
    if(y >0) fb(x,y-1,val^a[x][y],cnt+1);
}
int main() {
    fastio
    cin>>n>>m>>k;
    fr(i,n) for(LL j =0; j<m;++j) cin>>a[i][j];
    half = (n+m-2)/2;
    fa(0,0,0,0);  fb(n-1,m-1,0,0);
    cout<<ans<<endl;
	return 0;
}