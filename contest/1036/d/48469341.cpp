...
int main(){
    ...
    fastio
	int n,m; cin>>n; int a[n+1]; rep(i,1,n,1) cin>>a[i];
	cin>>m; int b[m+1]; rep(i,1,m,1) cin>>b[i]; int sum1=0, sum2=0;
	rep(i,1,n,1) sum1+=a[i]; rep(i,1,m,1) sum2+=b[i];
	if(sum1!=sum2){cout<<-1<<endl; return 0;}
	int p1 = 1, p2 = 1, len = 0;
	sum1=a[1], sum2=b[1];
	while(p1<=n and p2<=m){
		if(sum1 > sum2){sum2+=b[++p2];}
		else if(sum2 > sum1){sum1+=a[++p1];}
		else {sum2+=b[++p2]; sum1+=a[++p1]; len++;}
	}
	cout<<len<<endl;
    return 0;
}